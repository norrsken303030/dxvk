...
namespace dxvk {
  ...
  public:
    ...
    /**
     * \brief Queries swap chain status
     *
     * May acquire an image in order to check whether
     * subsequent present calls are likely to succeed.
     * \returns One of the following return codes:
     *  - \c VK_SUCCESS if an image is currently acquired and
     *        subsequent presents are likely to succeed.
     *  - \c VK_NOT_READY if no swap chain exists
     *  - An error if swap chain creation failed.
     */
